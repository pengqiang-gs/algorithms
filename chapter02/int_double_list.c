...
status list_insert(list* l, position p, type e)
{
	...
	if(0 == list_length(*l) && p == 0)
	{
		...
		n_new->_previous = l->_header;
		n_new->_next = NULL;
	}
	else
	{
		// look for the node whose position is p
		while(n_cur != NULL && pos < p)
		{
			n_cur = n_cur->_next;
			pos++;
		}

		n_new->_previous = n_cur->_previous;
		n_cur->_previous->_next = n_new;
		n_new->_next = n_cur;
		n_cur->_previous = n_new;
	}
	l->_size++;
...
status list_remove(list* l, position p, type* e)
{
	...
	*e = n_cur->_element;
	n_cur->_previous->_next = n_cur->_next;
	n_cur->_next->_previous = n_cur->_previous;
	free(n_cur);
